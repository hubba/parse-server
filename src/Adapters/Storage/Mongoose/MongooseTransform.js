...
// Returns the mongoose form of the query.
function transformWhere(className, restWhere, schema) {
  ...
  for (const restKey in restWhere) {
    ...
    mongooseWhere[out.key] = out.value;
  }
  return mongooseWhere;
}

const parseObjectKeyValueToMongooseObjectKeyValue = (restKey, restValue, schema) => {
  // Check if the schema is known since it's a built-in field.
...
const parseObjectToMongooseObjectForCreate = (className, restCreate, schema) => {
  restCreate = addLegacyACL(restCreate);
  const mongooseCreate = {}
  for (const restKey in restCreate) {
    if (restCreate[restKey] && restCreate[restKey].__type === 'Relation') {
      continue;
    }
    const { key, value } = parseObjectKeyValueToMongooseObjectKeyValue(
      restKey,
      restCreate[restKey],
      schema
    );
    if (value !== undefined) {
      mongooseCreate[key] = value;
    }
  }
...
// Helper function to transform an atom from REST format to Mongoose format.
...
// Transforms a query constraint from REST API format to Mongoose format.
...
// Transforms an update operator from REST format to mongoose format.
...
// the mongoose op, and arg being the argument.
...
function transformUpdateOperator({
  ...
  case 'AddUnique':
    ...
    if (flatten) {
      ...
      return {__op: mongooseOp, arg: {'$each': toAdd}};
    }
...
const nestedMongooseObjectToNestedParseObject = mongooseObject => {
  ...
  case 'symbol':
  case 'function':
    throw 'bad value in mongooseObjectToParseObject';
  case 'object':
    if (mongooseObject === null) {
      return null;
    }
    if (mongooseObject instanceof Array) {
      return mongooseObject.map(nestedMongooseObjectToNestedParseObject);
    }

    if (mongooseObject instanceof Date) {
      return Parse._encode(mongooseObject);
    }

    if (mongooseObject instanceof mongodb.Long) {
      return mongooseObject.toNumber();
    }

    if (mongooseObject instanceof mongodb.Double) {
      return mongooseObject.value;
    }

    if (BytesCoder.isValidDatabaseObject(mongooseObject)) {
...
const mongooseObjectToParseObject = (className, mongooseObject, schema) => {
  ...
  case 'object': {
    ...
    const restObject = {};
    if (mongooseObject._rperm || mongooseObject._wperm) {
      restObject._rperm = mongooseObject._rperm || [];
      restObject._wperm = mongooseObject._wperm || [];
      delete mongooseObject._rperm;
      delete mongooseObject._wperm;
    }
    ...
    for (var key in mongooseObject) {
      ...
      case '_id':
        ...
        break;
      case '__v':
        restObject['__v'] = '' + mongooseObject[key];
        break;
      case '_hashed_password':
        restObject._hashed_password = mongooseObject[key];
        break;
      case '_acl':
        break;
      case '_email_verify_token':
      ...
      case '_password_history':
        ...
        restObject[key] = mongooseObject[key];
        break;
      case '_session_token':
        restObject['sessionToken'] = mongooseObject[key];
        break;
      case 'updatedAt':
      case '_updated_at':
        restObject['updatedAt'] = Parse._encode(new Date(mongooseObject[key])).iso;
        break;
      case 'createdAt':
      case '_created_at':
        restObject['createdAt'] = Parse._encode(new Date(mongooseObject[key])).iso;
        break;
      case 'expiresAt':
      case '_expiresAt':
        restObject['expiresAt'] = Parse._encode(new Date(mongooseObject[key]));
        break;
      default:
        ...
        if (authDataMatch) {
          ...
          restObject['authData'][provider] = mongooseObject[key];
          break;
        }
        ...
        if (key.indexOf('_p_') == 0) {
          ...
          if (mongooseObject[key] === null) {
            break;
          }
          var objData = mongooseObject[key].split('$');
        ...
        restObject[key] = nestedMongooseObjectToNestedParseObject(mongooseObject[key]);
      }
    }
